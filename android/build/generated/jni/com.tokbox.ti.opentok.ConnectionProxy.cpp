...
Local<FunctionTemplate> ConnectionProxy::getProxyTemplate(Isolate* isolate)
{
	...
	javaClass = titanium::JNIUtil::findClass("com/tokbox/ti/opentok/ConnectionProxy");
...
, javaClass, nameSymbol);
	...
	// Method bindings --------------------------------------------------------
	titanium::SetProtoMethod(isolate, t, "getCreationTime", ConnectionProxy::getCreationTime);
	titanium::SetProtoMethod(isolate, t, "getConnectionId", ConnectionProxy::getConnectionId);
	...
	instanceTemplate->SetAccessor(NEW_SYMBOL(isolate, "creationTime"),
			ConnectionProxy::getter_creationTime,
			titanium::Proxy::onPropertyChanged,
			Local<Value>(), DEFAULT,
			static_cast<v8::PropertyAttribute>(v8::ReadOnly | v8::DontDelete)
		);
	instanceTemplate->SetAccessor(NEW_SYMBOL(isolate, "connectionId"),
			ConnectionProxy::getter_connectionId,
			titanium::Proxy::onPropertyChanged,
			Local<Value>(), DEFAULT,
			static_cast<v8::PropertyAttribute>(v8::ReadOnly | v8::DontDelete)
		);
...
// Methods --------------------------------------------------------------------
void ConnectionProxy::getCreationTime(const FunctionCallbackInfo<Value>& args)
{
	...
	if (!methodID) {
		methodID = env->GetMethodID(ConnectionProxy::javaClass, "getCreationTime", "()Ljava/util/Date;");
		if (!methodID) {
...
void ConnectionProxy::getConnectionId(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "getConnectionId()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(ConnectionProxy::javaClass, "getConnectionId", "()Ljava/lang/String;");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getConnectionId' with signature '()Ljava/lang/String;'";
			LOGE(TAG, error);
				titanium::JSException::Error(isolate, error);
				return;
		}
...
void ConnectionProxy::getter_creationTime(Local<Name> property, const PropertyCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
...
void ConnectionProxy::getter_connectionId(Local<Name> property, const PropertyCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
