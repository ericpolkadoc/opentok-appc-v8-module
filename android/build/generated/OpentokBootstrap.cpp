...
#define TAG "com.tokbox.ti.opentok"

using namespace v8;

static Persistent<Object> bindingCache;

static void Opentok_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Opentok.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::OpentokBindings::lookupGeneratedInit(
...
static void Opentok_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Opentok_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Opentok_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::OpentokBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry OpentokBinding = {
	"com.tokbox.ti.opentok",
	Opentok_init,
	Opentok_dispose
};
...
Java_com_tokbox_ti_opentok_OpentokBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.tokbox.ti.opentok", &OpentokBinding);
	titanium::KrollBindings::addExternalLookup(&(::OpentokBindings::lookupGeneratedInit));
}
